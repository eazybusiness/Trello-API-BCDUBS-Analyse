...
from trello_history_db import ingest_trello_cards, open_db_readonly
...
def _generate_navigation_menu(current_page: str) -> str:
    def cls(page: str) -> str:
        return "bg-blue-700 text-white" if current_page == page else "text-blue-100 hover:bg-blue-600 hover:text-white"

    return f"""
    <nav class=\"bg-gradient-to-r from-blue-600 to-indigo-700 shadow-lg mb-8\">
        <div class=\"max-w-7xl mx-auto px-4 sm:px-6 lg:px-8\">
            <div class=\"flex space-x-4 py-4\">
                <a href=\"speaker_workload_report.html\" class=\"{cls('workload')} px-4 py-2 rounded-md text-sm font-medium transition-colors\">🎤 Speaker Workload</a>
                <a href=\"completed_projects_report.html\" class=\"{cls('completed')} px-4 py-2 rounded-md text-sm font-medium transition-colors\">✅ Completed Projects</a>
                <a href=\"late_report.html\" class=\"{cls('late')} px-4 py-2 rounded-md text-sm font-medium transition-colors\">⏰ Late Report</a>
            </div>
        </div>
    </nav>
"""
...
def generate_late_report(
    db_path: str = "data/trello_history.sqlite",
    trello_json_path: str = "trello_cards_detailed.json",
    output_file: str = "reports/late_report.html",
) -> str:
    speaker_names = list(SPEAKER_PROFILES.keys())
    ingest_trello_cards(db_path=db_path, trello_json_path=trello_json_path, speaker_names=speaker_names)
...
<body class=\"bg-gray-50\">
    {_generate_navigation_menu('late')}

    <div class=\"max-w-7xl mx-auto px-4 sm:px-6 lg:px-8\">
        <div class=\"bg-white rounded-lg shadow p-6 mb-8\">
            <h1 class=\"text-2xl font-bold text-gray-900\">⏰ Late Report</h1>
            ...
            <div class=\"mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4\">
                ...
                <div class=\"p-4 rounded-lg bg-gray-50\">
                    ...
                    <div class=\"text-sm font-mono text-gray-900\">SQLite history</div>
                </div>
            </div>
        </div>
...
def main() -> None:
    out = generate_late_report()
    print(f"Late report generated: {out}")
